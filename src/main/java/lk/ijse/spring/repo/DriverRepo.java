...
public interface DriverRepo extends JpaRepository<Driver, String> {
    ...
    //SELECT * FROM driver WHERE nic NOT IN (SELECT DISTINCT driver_nic FROM car_reservation WHERE  (current_date() BETWEEN pick_up_date AND return_date OR pick_up_date=current_date() OR return_date=current_date()
    @Query(value = "SELECT * FROM driver WHERE nic NOT IN (SELECT DISTINCT driver_nic FROM car_reservation WHERE  (pick_up_date BETWEEN ?1 AND ?2 OR return_date BETWEEN ?1 AND ?2 OR pick_up_date=?1 OR return_date=?2)) ", nativeQuery = true)
    List<Driver> AvailableDrivers (String start_date,String end_date);
